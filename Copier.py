...
class Copier:
    def copy(self, file_path, dir_path):
        raise NotImplemented("Not implement Copier.copy")
...
class LimitDirSizeCopier(Copier):
    def __init__(self, max_files_in_dir=512, sub_dir_name_gen=None):
        ...
        if not sub_dir_name_gen:
            self._name_gen = NumberDirNameGenerator(0, max_files_in_dir)
    ...
    def copy(self, file_path, dir_path):
        ...
        cur_dir_path = os.path.join(dir_path, self._cur_name)
        ...
        shutil.copy(file_path, cur_dir_path)
